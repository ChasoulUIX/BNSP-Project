<x-app-layout>
    <x-slot name="header">
        <div>
            <h1 class="text-xl font-bold text-gray-900">Edit Barang</h1>
            <p class="text-sm text-gray-500 mt-0.5">Perbarui data barang</p>
        </div>
    </x-slot>
    ...
    <div class="max-w-xl">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h3 class="font-semibold text-gray-900">Form Edit: {{ $barang->nama_barang }}</h3>
            </div>
            <div class="p-6">
                @if ($errors->any())
                ...
                <form action="{{ route('barang.update', $barang->id) }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="number" id="harga" name="harga" value="{{ old('harga', $barang->harga) }}" min="0" required
                               class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition">
                    </div>
                    <div>
